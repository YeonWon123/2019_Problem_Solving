...
int main(void)
{
	...
	accu.push_back({ arr[0].counts, arr[0].counts * arr[0].point });
	for (int i = 1; i < arr.size(); i++)
		accu.push_back({ accu[i - 1].counts + arr[i].counts, accu[i - 1].point + arr[i].counts * arr[i].point });
	...
	for (int i = 0; i < arr.size(); i++) {
		...
		for (int j = i; j > -1; j--) {
			...
			int temp2;
			if (mid2 == 0) temp2 = 0;
			else if (j == 0) temp2 = accu[mid2 - 1].counts * mid - accu[mid2 - 1].point;
			else temp2 = (accu[mid2 - 1].counts - accu[j - 1].counts) * mid
				- (accu[mid2 - 1].point - accu[j - 1].point);

			part_answer += (temp2 * truck);
			...
			*/
			temp2 = (accu[i].counts - accu[mid2].counts) * mid
				- (accu[i].point - accu[mid2].point);
